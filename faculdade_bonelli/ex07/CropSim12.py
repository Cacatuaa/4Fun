...
import math
import sys
WorkDir = r'C:\Users\Cacatua\Desktop\Programacao\python\bonelli\ex07' + '\\'
...
# Interpolar os fatores de partição para o DVS específico 
   ...
   # Cálculo da ETp - ETc
    ...
    Evapo = Evapo * (Teta[1]-TetaPMP[1])/(TetaCC[1]-TetaPMP[1])
    
    # Fator de fracionamento do sistema radicular entre as camadas do solo
    for n in range(1,NCamadas+1):
        if Zcamada[n] <= Zedia:
...
#CÁLCULO DA REDUÇÃO DE TRANSPIRAÇÃO      ***
...
# Calcular nova úmidade (balanço hídrico por camada)
    ...
    for n in range(1,NCamadas+1):
        ...
        TetaL = TetaPMP[n]+(1-p)*(TetaCC[n]-TetaPMP[n]) 
        
        #print(Dia, n, Teta[n],TetaL)
        if Teta[n] >= TetaL:
            Srel = 1
        elif Teta[n] < TetaPMP[n]:
            Srel = 0
        else:
            Srel = (Teta[n] - TetaPMP[n]) / (TetaL - TetaPMP[n])
            
        if GrupoCultura == 0:   # Nesse caso calcula produtividade potencial
            Srel = 1
        ...
        Ta = Ta + Extracao
        Armaz[n] = Teta[n] * 10 * (Zcamada[n] - Zcamada[n-1])
    ...
    #print(Dia, NDia, Ta, Trans, Teta)

        
    # Valores totais, acumulados
    ChuvaTot = ChuvaTot + Chuva[Dia]
    TransTot = TransTot + Ta
    TransPotTot = TransPotTot + Trans
   ...
   # Calcular a respiração de manutenção 
    ...
    #Saída de valores diários
    print(Dia,round(DVS,3), round(IAF,3), round(Massa[3],1), round(Ta/Trans,3), round(Teta[1],3), round(Teta[2],3), Chuva[Dia],round(Trans,2), round(Ta,2), round(Evapo,2), round(Excedente,2))

#Saída de valores finais
print()
print('   Massa final grãos ', round(Massa[3],1))
print ('Chuva Total = ',round(ChuvaTot,1))
print ('Ta Total = ',round(TransTot,1))
print ('Evapo Total = ',round(EvapoTot,1))
print ('Excedente Total = ',round(ExcedTot,1))


# Calcular a produtividade da água WP
WP = Massa[3]/(EvapoTot+TransTot)/10   # em m3/kg
print('WP = ',round(WP,2), 'kg/m3')
